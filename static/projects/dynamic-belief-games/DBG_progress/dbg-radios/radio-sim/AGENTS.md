...
Rust + PyO3 tactical radio MANET MAC layer simulator with native TDMA/CSMA implementations, conformance harnessing, and mixed media scenario support.

## Runtime

- Rust: `cargo` (system install, `$HOME/.cargo/bin`)
- Python: prefer conda env `dev` unless session specifies otherwise; project venv `radio-sim/.venv` is also supported.
- Conda: `conda activate dev`
